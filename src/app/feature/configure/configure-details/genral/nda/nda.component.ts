...
export class NdaComponent implements OnInit, OnChanges {
  ...
  file: any;
  userDetails: any;
  docType: string = documentType.nda;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.ndaUploadFile) {
      this.uploadPdf();
    }
    // this.ndaUploadFile=false;
  }
  uploadPdf() {
    let path = "level1/" + this.userDetails?.level1DisplayId + "/Nda/" + new Date().getTime() + "/";
