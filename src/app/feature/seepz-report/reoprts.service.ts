...
export class ReoprtsService {
  ...
  seepzPassReportData(reqObj): Observable<ReportsResponse> {
    return this._http.post<any>(environment.CompanyAPIURL + this.restApi.SEEPZ_PASS_REPORT_ASYNC, reqObj)
      .pipe(map(data => {
        return data;
      }));
  }
  exportSeepsReportData(reqObj) {
    ...
    return this._http.post<any>(environment.CompanyAPIURL + this.restApi.EXPORT_SEEPZ_REPORTS_DATA, reqObj, { headers: headers, responseType: 'blob' as 'json' })
      .pipe(map(data => {
        return data
      }));
  }
  ...
  getVisitorType(){
   ...
   changeLocation(reqBody) {
    ...
    viewReportDetailss(reqObj): Observable<ReportsResponse> {
      return this._http.post<any>(environment.CompanyAPIURL + this.restApi.REPORT_DETAIL_VIEW, reqObj)
        .pipe(map(data => {
          return data;
        }));
    }
